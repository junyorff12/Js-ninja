(function(window, document){
	...
	*/
	...
	function isLastItemOperation( stringNumber ){
		let operations = ['+', '-', 'x', '/'];
		let lastItem = stringNumber.split('').pop();
		return operations.some( operator => {
			return lastItem === operator;
		});
	}

	function removeLastItemIfOperation( stringNumber ){
		if( isLastItemOperation( stringNumber )){
			return stringNumber = stringNumber.slice(0, -1);
		}
		return stringNumber;
